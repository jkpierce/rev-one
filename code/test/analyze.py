...
bins = np.hstack((np.array([0]),np.geomspace(1e-7,5e7,num=50)))
...
def compute_crt(te, me, td, md, mv, bins = bins):
    ...
    # H is now the number of return times in each bin
    # but you want the cumulative number of return times associated to each bin
    H = H.cumsum() # and there it is
    if H.max()>0:
    	H = H/H.max() # and normalize it if it's non-empty.. otherwise leave it all zeros
    return H
...
def compute_cdf(inputs, bins=bins):
    ...
    for mv,p in zip(m, pm): # iterate through all mvalues in question
        conditional_cdf = compute_crt(te, me, td, md, mv)
        cdf+=conditional_cdf*p # probability of the elevation occuring times the counts at that elevation
    # now manipulate cdf so it's the cumulative non-exceedance probability
    cdf = 1.0 - cdf
...
# done. The output is called rt_pdf

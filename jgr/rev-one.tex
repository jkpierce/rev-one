...
\title{Joint stochastic theory of bedload transport and bed elevation: derivation of heavy-tailed resting times}
...
\begin{figure}
  ...
  \vspace{-1.0cm}
  \caption{Definition sketch of a control volume containing $n$ moving grains and $m$ resting grains. Migration, entrainment, and deposition processes are represented by curved arrows, and the bed elevation at the instant of the figure (in a degraded state) is depicted by dotted line. The distributions of $n$ and $m$ are indicated in the upper right panel, while the bottom panel is a time-series of bed elevations (\ref{eq:ele}).}
  \label{fig:concept}
\vspace{-0.75cm}
\end{figure}
...
\end{align}
These rates are independent of the past history of the process and depend only on its current state $(n,m)$. 
This means the system satisfies the Markov hypothesis \citep[e.g.][]{Cox1965}, implying time intervals between subsequent transitions are exponentially distributed \citep[e.g.][]{Gillespie2007}.
...
In contrast, $\lambda(m)$, $\mu(m)$, and $\sigma(m)$, characterizing entrainment, collective entrainment, and deposition rates per grain are allowed to depend on $m$ through the bed elevation (\ref{eq:ele}).
Collective entrainment has been discussed carefully in many studies \citep[e.g.][]{Ancey2008, Heyman2013, Heyman2014, Ma2014, Ancey2014a}, so we won't discuss on this topic. We only mention \citet{Ancey2008} introduced it as a means to describe realistically wide bedload fluctuations.
...
\end{align}
In these expressions, the ratio $z_1/l$ controls the sensitivity of these rates to the addition or removal of a single grain.
...
As we'll see, the variance of bed elevation turns out to be given by $\text{var}(z) = (l z_1)^2$, so at the end of the day, $l$ characterizes the range of bed elevation variation, which could be considered the depth of the active layer of the bed \citep[e.g.][]{Church2017}.
...
\{ \nu + \lambda(m) + n\mu(m) + n\sigma(m) + n \gamma \}P(n,m;t).
 \label{eq:master}
\end{multline} 
The solution $P(n,m;t)$ of this equation provides the statistics of $n$ and $m$.
...
In our case, the two species are the moving and stationary grains in the volume which are proxy variables for the bedload flux and bed elevation.

\section{Numerical simulations}

Unfortunately, (\ref{eq:master}) does not admit an analytical solution, at least by methods applied to similar systems in the population ecology literature \citep[e.g,][]{Swift2002} or the predecessor of this model \citep{Ancey2008}.
This difficulty results from the product terms between $n$ and $m$ in (\ref{eq:master}).
To gain understanding of the stochastic evolution of the populations $n$ and $m$, we resort to numerical methods. For our simulations, we chose the Gillespie algorithm (\ref{eq:master}) \citep{Gillespie1977, Gillespie1992, Gillespie2007}.
. 
...
The Gillespie algorithm leverages the defining property of a Markov process: the time interval between subsequent transitions is exponentially distributed \citep[e.g.][]{Cox1965}.
Using this property, it computes exact realizations of the stochastic process from which any statistics of interest can be discerned, providing all the information contained in (\ref{eq:master}).
To step the Markov process through a single transition, it's enough to draw a random value from this exponential distribution to determine the time at which the transition occurs, and to choose the transition's type (entrainment, migration, deposition) using another uniform random value between $0$ and $1$, selecting among the relative rates of these possible transitions.
After this first transition is enacted, i.e., by stepping $n$ and $m$ by the shifts associated with the type of transition which was selected, this two-stage selection is repeated arbitrarily, forming an exact realization of the stochastic process \citep{Gillespie1977, Gillespie1992, Gillespie2007}.

Using this algorithm, we simulated 5 flow conditions with 8 different values of $l$, taken across a range from one granular radius ($l=a$) to 8 radii, for a total of 40 simulations.
Apart from $l$, the parameters used for our numerical experiments are taken from the flume trials of \citet{Ancey2008}. These values are summarized in table \ref{tab:anceyparams}.
They are labeled (a), (g), and so on, in order of increasing bedload transport rate (see \citet{Ancey2008} for more details). 
...
Each simulation was run for $1500$hrs of virtual time, which we selected by trial and error to ensure convergence of the statistics.
...
\begin{wrapfigure}{r}{0.5\textwidth}
	...
	\caption{Since $l$ controls the standard deviation of bed elevations, we can interpret it as the depth of the active layer \citep[e.g.][]{Church2017}.}
	\label{fig:var}
\end{wrapfigure}


\begin{enumerate}
	\item describe calculation of bedload transport and bed elevation statistics
	\item show graphs of bedload transport and bed elevation statistics
	\item describe calculation of resting time distribution
	\item show graphs of resting time distributions
	\item describe physical reasoning to derive the timescale for scaling 
	\item describe the manner of the collapse and the influence of E and l in this 
\end{enumerate}
...
We generalized the stochastic birth-death model of \citet{Ancey2008} to include a second population: the number of particles at rest on the bed.
To our knowledge, this is a new and unique stochastic description of the coupling between sediment transport and bed elevation changes.
Interpreting the number of particles at rest as a proxy for the bed elevation and the resting times of sediment undergoing burial as return times from above in the bed elevation time-series \citep[e.g.][]{Voepel2013,Martin2014}, we compute heavy-tailed resting time distributions with tail parameter $\alpha \approx 1.15$, implying bedload super-diffusion \citep[e.g.][]{Weeks1998,Bradley2017}. 

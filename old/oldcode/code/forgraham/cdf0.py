...
print('filenameshort')
def load(filename, chunksize = 50000):
...
def conditional_rt(entrainfile, depositfile, mstar, bins = bins, dt = 0.5, tmax = tmax):
    ...
    # make sure you start from a departure and end at a return
    while returns[0] < departures[0]: # while first return smaller than first departure
        returns = returns[1:] # crop it 
    while departures[-1] > returns[-1]: # while last departure greater than last return 
        departures = departures[:-1] # crop it

    # at this point, the return time series starts from a departure
    # and ends from a return 
    # but there is no guarantee that it doesn't
    # start with k departures or k returns 
        
    # so if it isn't woven as |d,r,d,r,d,r,d,r| ...  that's because 
    # (a) there are two or more departures prior to the first return
    #     as in |d,d,d,r,d,r,d,r,d,r,d,r| ... or 
    # (b) there are two or more returns post the last departure,
    #     as in |d,r,d,r,d,r,r,r,r| .... 
    ...
    F = 1-np.cumsum(H)*dt # cumulative distribution of return time  
    return F 

# get the statistics of m 
cdffilename = filenameshort[:-4]+'-rtcdf.dat' # filename of the cdf

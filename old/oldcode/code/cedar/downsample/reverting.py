...
def simulate(param, tmax):
    """gillespie algorithm until simulation reaches tmax"""
    t = 0
    n = n0
    m = m0
    S = []
    t1 = 3600 # time to print status at
    # unpack params to compute sampling rate
    nu,la0,la1,si0,si1,mu0,mu1,ga = itemgetter('nu','la0','la1','si0','si1','mu0','mu1','ga')(param)

    ### compute how much to downsample in order to control output size
    maxmem = 2e9 # max number of bytes of the stored simulation
    #maxmem=0.01e9 #100Mb memory cap for testing
    mem1 = 8*3  # 8 bytes/item  memory of saving 1 point in the simulation
    # estimated mean transition rate from the initial conditions #/sec.. total mean propensity
    nbar = (nu+la0)/(ga+si0-mu0) # mean expected from ancey2008 lambda/(alpha-mu).. lambda = nu + la0, alpha = ga + si0
    print('expected number in motion is {}'.format(round(nbar,2)))
    ...
    print('{} transitions expected in the course of the simulation'.format(int(round(nt))))
    totmem = nt*mem1 # total memory if all transitions sampled
    ns = int(round(totmem/maxmem))  # number of transitions to wait before sampling
    if ns<1: ns=1 # make sure it's at least 1
    print('expected memory of entire simulation is {}Gb, so sampling every {} points'.format(round(totmem/1e9,3),round(ns)))
    del maxmem, mem1, rt, nt, totmem, nbar

    i=ns
    i0 = np.copy(ns)
    while t<tmax:
        i+=1 # counter for saving the simulation output
        ...
        if i>i0: # save every ns transitions
            i0+=ns # in this way the output should have about size maxmem

...
def Lam(n,m,params):
    ...
    nu,la0,la1,si0,si1,mu0,mu1,ga = itemgetter(
        'nu','la0','la1','si0','si1','mu0','mu1','ga')(params)
    out = la0 + la1 * z(m)
    if out < 0:
...
def simulate(param, tmax):
    """run a gillespie algorithm simulation
    start at t=0,n=n0,m=m0 
    print the status every t1 seconds
    save the state every t2 seconds 
    return n,m,t 
    where n is the timeseries of activities
    m is the timeseries of bed elevations
    t is the set of times over which they're defined"""
    t = 0
    n = n0
    m = m0 
    S = []

...
n, m, t = np.load(filename) # load the file
...
def compute_pdf(inputs, bins=bins):
    te, me, td, md, m, pm = inputs # these come from loader
    pdf = np.zeros(len(bins)-1,dtype=float) # sum into this iteratively
    for mv,p in zip(m, pm): # iterate through all mvalues in question
        c_pdf = compute_crt(te, me, td, md, mv)
        pdf+=c_pdf*p
    data = np.array((bins[1:], pdf))
    return data

inputs = te, me, td, md, m_unique, pm
rt_pdf = compute_pdf(inputs).T
...
output = {'filename':filename, 'l':l, 'parameters': params, 'n_pdf':pdf_n, 'm_pdf':pdf_m, 'rt_pdf':rt_pdf, 'ancey_n_stats':{'n_mean':anc_n_mean, 'n_var':anc_n_var}, 'E': E, 'a': a, 'duration':duration}
